...
def startAppHandle():
    ...
    # Start user process
    userApp = subprocess.Popen([decryptedFileRunPath], shell=True, **subprocess_args(False))
    user_app_pid = userApp.pid

    # Start supervisor process
    supervisor_cmd = ['.\MySupervisor.exe', decryptedFileRunPath, str(my_engine_pid), str(user_app_pid)]
    supervisor_proc = subprocess.Popen(supervisor_cmd, **subprocess_args(False))
...
def supervisorProcRun(runPath, my_engine_pid, user_app_pid):
